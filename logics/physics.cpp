...
void elastic_collision( point *p_i, point *p_j )
{
    ...
    p_i->vel = ((n * ((p_i->mass - p_j->mass) * (v_i * n) + 
        2 * p_i->mass * (v_j * n)) / (p_i->mass + p_j->mass)) + 
        (m * (v_i * m)));
    p_j->vel = ((n * ((p_j->mass - p_i->mass) * (v_j * n) +
        2 * p_j->mass * (v_i * n)) / (p_i->mass + p_j->mass)) + 
        (m * (v_j * m)));
}
...
void inelastic_collision( point *p_i, point *p_j )
{
    ...
    m.x = n.y;
    m.y = -n.x;
    p_i->vel = (n * (p_i->mass * (v_i * n) + p_j->mass * (v_j * n)) / 
        (p_i->mass + p_j->mass)) + (m * (v_i * m));
    p_j->vel = (n * (p_i->mass * (v_i * n) + p_j->mass * (v_j * n)) / 
        (p_i->mass + p_j->mass)) + (m * (v_j * m));
}
...
void adhesion( point *p_i, point *p_j )
{
    ...
    p_i->vel = ((v_i * p_i->mass) + (v_j * p_j->mass)) / 
        (p_i->mass + p_j->mass);
    p_i->pos = ((p_i->pos * p_i->mass) + (p_j->pos * p_j->mass)) / 
        (p_i->mass + p_j->mass);
